...
def weighted_cross_entropy_with_logits(FLAGS, Fold, logits, labels):
    ...
    """
    print("Test:" + str(logits.shape) + "," + str(labels.shape))
    ...
    weights_1 = tf.convert_to_tensor([1, 1])
    weights_2 = tf.convert_to_tensor([1, 1])
    weights_3 = tf.convert_to_tensor([1, 1])
    weights_4 = tf.convert_to_tensor([1, 1])
    weights_5 = tf.convert_to_tensor([1, 1])
    weights_all = [weights_1,weights_2,weights_3,weights_4,weights_5]
    
    #weights = weights_'%d' %Fold
    weights = weights_all[Fold-1]
    print("Test:" + str(softmax.shape) + "," + str(labels.shape))
